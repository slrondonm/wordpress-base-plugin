 ...
 * 
 * @author Ing. Sergio Rondón | Grupo Virtualizate (@slrondonm)
 * @since 0.0.3
 */
var pkg = require('./package.json');
/**
 * Configuration of paths and global arrays
 */


var project = pkg.name;
var cssOutputStyle = 'compressed';
var cssOutputComments = false;
...
var jsTasks = [{
  name: 'frontend',
  suffix: false,
  include: [jsSourcePath + 'common/**/*.js'] //source: jsSourcePath + 'frontend', // Optional subdirectory to search for *.js. Defaults to 'name' value
...
var renameStrings = [['slrondonm\/wordpress-base-plugin', pkg.config.username + '/' + pkg.name], // Git/Composer identifier
['Virtualizate\\PluginName', pkg.config.php_namespace], // PHP namespace for your plugin
['Virtualizate\\\\PluginName', pkg.config.php_namespace.replace(/\\/g, '\\\\')], // Rename Composer namespace
...
var AUTOPREFIXER_BROWSERS = ['last 2 version', '> 1%', 'ie >= 9', 'ie_mob >= 10', 'ff >= 30', 'chrome >= 34', 'safari >= 7', 'opera >= 23', 'ios >= 7', 'android >= 4', 'bb >= 10'];
/**
...
var pkg = require('./package.json'); // CSS-related plugins
...
var errorHandler = function errorHandler(r) {
  notify.onError('\n\n ===> ERROR: <%= error.message %>\n')(r);
  beep();
};
...
styleTasks.forEach(function (task) {
  ...
  gulp.task(task.name + 'CSS', function (done) {
    gulp.src(styleSourcePath + task.source).pipe(plumber(errorHandler)).pipe(sourcemaps.init()).pipe(sass({
      sourceComments: cssOutputComments ? 'map' : null,
      errLogToConsole: true,
      outputStyle: cssOutputStyle,
      precision: 10
    })).on('error', console.error.bind(console)).pipe(sourcemaps.write({
      includeContent: false
    })).pipe(sourcemaps.init({
      loadMaps: true
    })).pipe(autoprefixer(AUTOPREFIXER_BROWSERS)).pipe(sourcemaps.write(styleMapPath)).pipe(rename({
      basename: project + basename_suffix
    })).pipe(lineec()) // Consistent Line Endings for non UNIX systems.
    .pipe(gulp.dest(styleDestination)).pipe(filter('**/*.css')) // Filtering stream to only css files
    .pipe(mmq({
      log: true
    ...
    .pipe(rename({
      basename: project + basename_suffix,
      suffix: '.min'
    })).pipe(minifycss()).pipe(lineec()) // Consistent line endings for non-UNIX systems.
    .pipe(gulp.dest(styleDestination)).pipe(filter('**/*.css')) // Filtering stream to only css files
    .pipe(notify({
      message: 'TASK: "' + task.name + 'CSS" completed.',
      onLast: true
    }));
    done();
  });
...
jsTasks.forEach(function (task) {
  ...
  var basename_suffix = typeof task.suffix === 'string' ? '-' + task.suffix : '';
  if (!(typeof task.suffix === 'boolean' && task.suffix === false)) basename_suffix = '-' + task.name; // Set base filename suffix (example '-admin'), if specified

  var jsSources = [(task.source ? task.source : jsSourcePath + task.name) + '/*.js']; // Set JavaScript source paths
  ...
  gulp.task(task.name + 'JS', function (done) {
    gulp.src(jsSources).pipe(plumber(errorHandler)).pipe(concat(task.source + '.js')).pipe(rename({
      basename: project + basename_suffix
    })).pipe(lineec()) // Consistent Line Endings for non UNIX systems.
    .pipe(gulp.dest(jsDestination)).pipe(rename({
      basename: project + basename_suffix,
      suffix: '.min'
    })).pipe(uglify()).pipe(lineec()) // Consistent Line Endings for non UNIX systems.
    .pipe(gulp.dest(jsDestination)).pipe(notify({
      message: 'TASK: "' + task.name + 'JS" completed.',
      onLast: true
    }));
    done();
  });
...
gulp.task('default', gulp.series(gulp.parallel(object_property_to_array(tasks_js, 'id', tasks_css)), function () {
...
gulp.task('rename', function () {
  return gulp.src(['./**/*.php', './*.json', './**/*.js', './**/*.scss', './*.txt', './*.md', '!./node_modules/**', '!./vendor/**', '!./.git/**', '!./languages/**', '!./*lock*', '!./gulpfile.js']).pipe(plumber(errorHandler)).pipe(replace(renameStrings)).pipe(vinylPaths(del)).pipe(batchRename(function (path) {
    return path.replace(/wordpress-base-plugin/, pkg.name);
  })).pipe(gulp.dest('./')).pipe(notify({
    message: 'TASK: "rename" completed.',
    onLast: true
  }));
});
/**
